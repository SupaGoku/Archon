...
import { MCPClient, MCPClientConfig, mcpClientService } from '../../services/mcpClientService'
      ...
      setClients((prevClients) =>
        prevClients.map((client) => {
          ...
          if (dbClient) {
            return {
              ...client,
              status: dbClient.status === 'connected' ? 'online' : dbClient.status === 'error' ? 'error' : 'offline',
    ...
    <div className="relative">
      ...
      {error && (
        <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
          ...
          <button onClick={() => setError(null)} className="text-red-500 hover:text-red-600 text-sm mt-2">
            Dismiss
          </button>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">Connect and manage your MCP-enabled applications</p>
        </div>
        <Button
      ...
      {isEditDrawerOpen && editClient && (
        <EditClientDrawer
          ...
          onUpdate={(updatedClient) => {
            ...
            setClients((prev) => {
              if (!updatedClient) {
                // If updatedClient is null, it means deletion
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white/90 dark:bg-black/90 border border-gray-200 dark:border-gray-800 rounded-lg p-6 w-full max-w-md relative backdrop-blur-lg">
        ...
        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Add New MCP Client</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* MCP Server URL */}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">MCP Server URL *</label>
            <input
              type="text"
              value={formData.url}
              ...
              required
            />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">The HTTP endpoint URL of the MCP server</p>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button type="submit" variant="primary" accentColor="cyan" disabled={isSubmitting}>
        ...
        <form onSubmit={handleUpdateSubmit} className="space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button type="button" variant="ghost" accentColor="pink" onClick={handleDelete}>
                Delete Client
              </Button>
              <Button
                type="button"
                variant="ghost"
                accentColor="cyan"
                onClick={() => window.open(`${API_BASE_URL}/mcp/clients/${client.id}/status`, '_blank')}
              >
                Debug Status
              </Button>
            </div>
